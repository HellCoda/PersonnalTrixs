...
public class ListDoubleFloat implements IListDoubleFloat {

	private List<List<ValeurFloat>> valeurs;
	private int sousTailleActuelle; // Pour garder une trace du nombre de colonnes

	public ListDoubleFloat(int sousTaille) {
		this.valeurs = new ArrayList<>();
		this.sousTailleActuelle = sousTaille;
	}

	@Override
	public void ajouterLigne(List<ValeurFloat> nouvelleLigne) {
	...
	@Override
	public ValeurFloat obtenir(int ligne, int colonne) {
		if (ligne >= 0 && ligne < valeurs.size() && colonne >= 0 && colonne < sousTailleActuelle) {
			List<ValeurFloat> ligneListe = valeurs.get(ligne);
	...
	public void afficher() {
		...
		for (int i = 0; i < valeurs.size(); i++) {
			System.out.print("[");
			List<ValeurFloat> ligneListe = valeurs.get(i);
